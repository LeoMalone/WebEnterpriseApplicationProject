...
public class EditUser {
	 ...
	 * @return status - boolean value
	 */
	public static boolean activateUser(UserBean user) {
		...
		try {
			conn = ConnectionManager.getConnection();
			updateUser = conn.prepareStatement("UPDATE users SET adminActivated=? WHERE userID=?");

			updateUser.setBoolean(1, true);
			updateUser.setString(2, user.getId());
