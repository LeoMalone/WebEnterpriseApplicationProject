 ...
 */
public class RefCreateScheduleServlet extends HttpServlet {
		...
		if (!(request.getSession().getAttribute("signedIn").equals("Referee"))) {
			...
			// Set cookie language for users
				...
				if(AdminTeams.teamsForEditSchedule(teamList)) {						
					...
					RequestDispatcher rd = request.getRequestDispatcher("ref_create_schedule.jsp");  
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {	
		...
		if(newDate == null || newTime == null || newHomeTeam == null || newHomeScore == "" || newAwayTeam == null || newAwayScore == "" || newGameStatus == null) {
			response.sendRedirect("./refCreateSchedule");
		} else {
			...
			if(AdminSchedule.createNewGame(schedule)) {
				response.sendRedirect("./refSchedule");
			}
		}
	}
}
